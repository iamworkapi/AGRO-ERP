...
import { getStoredDispatches } from "../features/biomass/biomassService";
...
export default function BiomassBuyers() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const { buyers: list } = await fetchBuyers({ limit: 200 });
        if (!cancelled) setBuyers(list);
      } catch (err) {
        if (!cancelled) toast.error("Failed to load buyers.");
      } finally {
  ...
  async function handleSaveEdit(e) {
    ...
    try {
      const updated = await apiUpdateBuyer(editingBuyer.id, {
      ...
      setBuyers((prev) => prev.map((b) => (b.id === editingBuyer.id ? { ...b, ...updated } : b)));
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
      <PageHeader
        title="Industrial Buyer Directory"
        ...
        <>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 16 }} className="responsive-grid-1">
              ...
              {filteredBuyers.length === 0 && (
                <div style={{ gridColumn: "span 2", textAlign: "center", padding: "40px 20px", color: "var(--muted)" }}>
                  ...
                  {loading ? "Loading buyers..." : `No buyers found matching "${searchTerm}"`}
                </div>
              )}
            </div>
          )}
          ...
          {selectedBuyerForDetails && (
            ...
            >
              <div
                style={{
                  background: "var(--surface)", border: "1.5px solid var(--line-strong)", borderRadius: 16,
              ...
              >
                <div style={{ padding: "16px 20px", borderBottom: "1px solid var(--line)", display: "flex", justifyContent: "space-between", alignItems: "center", background: "var(--surface-tint)" }}>
                  <div>
                  ...
                  <button onClick={() => setSelectedBuyerForDetails(null)} style={{ background: "none", border: "none", fontSize: 20, cursor: "pointer", color: "var(--muted)" }}>
                    ✕
                  </button>
                </div>
          ...
          {editingBuyer && (
            ...
            >
              <div
                style={{
                  background: "var(--surface)", border: "1px solid var(--line-strong)", borderRadius: 16,
