...
import { dateRangeCheck, dateRangeErrorMessage } from "../validators/formatValidators";
...
const LEAVE_ICONS = {
  ...
  "Maternity Leave": "ri-women-line",
  "Paternity Leave": "ri-men-line",
  "Unpaid Leave": "ri-money-dollar-circle-line",
};
...
function labelForLeaveType(value) {
  const map = { casual: "Casual Leave", sick: "Sick Leave", earned: "Earned Leave", maternity: "Maternity Leave", paternity: "Paternity Leave", unpaid: "Unpaid Leave", other: "Emergency Leave" };
  return map[value] || "Casual Leave";
}
...
export default function LeaveRequests() {
  ...
  const defaultWarehouseId = myWarehouse?.id || "";
  ...
  // Fill in the scoped warehouse once it resolves.
  if (isScopedRole && !form.warehouseId && defaultWarehouseId) {
    setForm((f) => (f.warehouseId ? f : { ...f, warehouseId: defaultWarehouseId }));
  }
  ...
  const totalCount = leaveRequests.length || 1;
  ...
  async function handleSubmit(e) {
    ...
    if (!form.employeeId || !form.reason) {
      toast.error("Please fill in employee and leave reason.");
      return;
    }
    if (form.fromDate && form.toDate && new Date(form.toDate) < new Date(form.fromDate)) {
      toast.error(dateRangeErrorMessage);
      return;
    }
  ...
  const displayType = labelForLeaveType(form.leaveType);

  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      <PageHeader
        title={isScopedRole ? `Employee Leave Management — ${myWarehouse?.name || "your hub"}` : "Employee Leave Management"}
        subtitle={
          isScopedRole
            ? `Review, approve, and track employee leave applications for ${myWarehouse?.name || "your assigned warehouse"}`
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 14 }} className="responsive-grid-2">
        ...
        <div
          ...
          style={{
            background: "var(--surface)", border: statusFilter === "Pending" ? "2px solid #F59E0B" : "1px solid rgba(245,158,11,0.2)",
            borderRadius: 16, padding: "16px 18px",
            boxShadow: statusFilter === "Pending" ? "0 12px 30px -4px rgba(245, 158, 11, 0.25)" : "0 6px 20px -2px rgba(0,0,0,0.04)",
            cursor: "pointer", position: "relative", overflow: "hidden", transition: "all 0.25s cubic-bezier(0.16, 1, 0.3, 1)",
          }}
        >
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
            ...
            <span style={{ fontSize: 10.5, fontWeight: 700, padding: "2px 7px", borderRadius: 10, background: "#FEF3C7", color: "#D97706", border: "1px solid rgba(245,158,11,0.3)" }}>{pendingPct}% Action Needed</span>
          </div>
        ...
        <div
          ...
          style={{
            background: "var(--surface)", border: statusFilter === "Approved" ? "2px solid #10B981" : "1px solid rgba(16,185,129,0.2)",
            borderRadius: 16, padding: "16px 18px",
        ...
        >
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
            <span style={{ fontSize: 11, fontWeight: 700, color: "var(--muted)", textTransform: "uppercase", letterSpacing: 0.4 }}>Approved Leaves</span>
            <span style={{ fontSize: 10.5, fontWeight: 700, padding: "2px 7px", borderRadius: 10, background: "#D1FAE5", color: "#059669", border: "1px solid rgba(16,185,129,0.3)" }}>{approvedPct}% Granted</span>
          </div>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <div>
        ...
        <div
          ...
          style={{
            background: "var(--surface)", border: statusFilter === "Rejected" ? "2px solid #EF4444" : "1px solid rgba(239,68,68,0.2)",
            borderRadius: 16, padding: "16px 18px",
        ...
        >
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
            <span style={{ fontSize: 11, fontWeight: 700, color: "var(--muted)", textTransform: "uppercase", letterSpacing: 0.4 }}>Disapproved / Declined</span>
            <span style={{ fontSize: 10.5, fontWeight: 700, padding: "2px 7px", borderRadius: 10, background: "#FEE2E2", color: "#EF4444", border: "1px solid rgba(239,68,68,0.3)" }}>{rejectedPct}% Rejected</span>
          </div>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <div>
        ...
        <div
          ...
          style={{
            background: "var(--surface)", border: statusFilter === "all" ? "2px solid #3B82F6" : "1px solid rgba(59,130,246,0.2)",
            borderRadius: 16, padding: "16px 18px",
        ...
        >
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
            <span style={{ fontSize: 11, fontWeight: 700, color: "var(--muted)", textTransform: "uppercase", letterSpacing: 0.4 }}>Total Applications</span>
            <span style={{ fontSize: 10.5, fontWeight: 700, padding: "2px 7px", borderRadius: 10, background: "#EFF6FF", color: "#2563EB", border: "1px solid rgba(59,130,246,0.3)" }}>100% Total</span>
          </div>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <div>
              <div style={{ fontSize: 22, fontWeight: 800, color: "var(--ink)", textShadow: "0 2px 10px rgba(59,130,246,0.2)" }}>{leaveRequests.length}</div>
      ...
      <div>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12, flexWrap: "wrap", gap: 10 }}>
          <div className="role-picker-container" style={{ width: "auto", marginBottom: 0, background: "var(--surface)", border: "1px solid var(--line)", padding: 3, borderRadius: 10 }}>
            <button type="button" className={`role-picker-option ${statusFilter === "all" ? "active" : ""}`} onClick={() => setStatusFilter("all")} style={{ padding: "6px 14px", fontSize: 12, fontWeight: 700, display: "inline-flex", alignItems: "center", gap: 6 }}>
              <i className="ri-list-check" style={{ fontSize: 11 }} /> All Applications ({leaveRequests.length})
            </button>
            <button type="button" className={`role-picker-option ${statusFilter === "Pending" ? "active" : ""}`} onClick={() => setStatusFilter("Pending")} style={{ padding: "6px 14px", fontSize: 12, fontWeight: 700, display: "inline-flex", alignItems: "center", gap: 6 }}>
              <i className="ri-time-line-rotate-left" style={{ fontSize: 11 }} /> Pending ({pendingCount})
            </button>
            <button type="button" className={`role-picker-option ${statusFilter === "Approved" ? "active" : ""}`} onClick={() => setStatusFilter("Approved")} style={{ padding: "6px 14px", fontSize: 12, fontWeight: 700, display: "inline-flex", alignItems: "center", gap: 6 }}>
              <i className="ri-checkbox-circle-fill" style={{ fontSize: 11 }} /> Approved ({approvedCount})
            </button>
            <button type="button" className={`role-picker-option ${statusFilter === "Rejected" ? "active" : ""}`} onClick={() => setStatusFilter("Rejected")} style={{ padding: "6px 14px", fontSize: 12, fontWeight: 700, display: "inline-flex", alignItems: "center", gap: 6 }}>
        ...
        <DataTable
          ...
          right={
            <Button className="btn-glow" onClick={() => openModal()} style={{ padding: "7px 14px", fontSize: 12.5, fontWeight: 700, display: "inline-flex", alignItems: "center", gap: 6, background: "var(--gradient-primary)", boxShadow: "0 3px 10px rgba(0, 184, 107, 0.3)" }}>
          ...
          columns={[
            {
              key: "employee", label: "Employee", emphasize: true,
            ...
            },
            {
              ...
              render: (r) => (
                <span style={{ fontWeight: 600, color: "var(--primary-deep)", display: "inline-flex", alignItems: "center", gap: 5 }}>
                  ...
                  {r.warehouse || (isScopedRole ? myWarehouse?.name : "")}
                </span>
              ),
            },
            {
              key: "dates", label: "Leave Schedule",
              render: (r) => (
            ...
            {
              key: "reason", label: "Reason / Notes", sortable: false,
              render: (r) => (
            ...
            },
            {
              ...
              render: (r) => <Badge tone={LEAVE_TONES[r.status] || "warning"}>{r.status ? r.status.toUpperCase() : "PENDING"}</Badge>,
            },
            {
              ...
              render: (r) => (
                <div style={{ display: "flex", gap: 6 }}>
                  {r.status === "Pending" ? (
                    <>
                      <Button variant="secondary" disabled={busyId === r.id} onClick={() => handleApprove(r)} style={{ padding: "4px 10px", fontSize: 11, fontWeight: 700, display: "inline-flex", alignItems: "center", gap: 4, background: "var(--primary-tint)", color: "var(--primary-deep)", borderColor: "var(--primary)" }}>
                        <i className="ri-check-line" /> Approve
                      </Button>
                      <Button variant="secondary" disabled={busyId === r.id} onClick={() => handleReject(r)} style={{ padding: "4px 10px", fontSize: 11, fontWeight: 600, display: "inline-flex", alignItems: "center", gap: 4, color: "#EF4444", borderColor: "#FEE2E2", background: "#FEF2F2" }}>
      ...
      <Modal open={open} title="Apply for Employee Leave" subtitle="Submit a formal leave request for warehouse staff or field employees" onClose={() => closeModal()}>
        <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          <FormField label="Employee Name" type="select" required icon="ri-user-3-line" value={form.employeeId} onChange={set("employeeId")} options={employeeOptions} placeholder="Select staff member" compact marginBottom={10} />

          <FormField label="Warehouse Hub" type="select" required disabled={isScopedRole} icon="ri-building-line" value={form.warehouseId} onChange={set("warehouseId")} options={warehouses.map((w) => ({ value: w.id, label: w.name }))} compact marginBottom={10} />

          <FormField label="Leave Category" type="select" required icon="ri-stack-line" value={form.leaveType} onChange={set("leaveType")} options={[
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0 12px" }} className="responsive-grid-2">
            <FormField label="Start Date" type="date" icon="ri-calendar-line-day" value={form.fromDate} onChange={set("fromDate")} compact marginBottom={10} />
            <FormField label="End Date" type="date" icon="ri-calendar-line-day" value={form.toDate} onChange={set("toDate")} compact marginBottom={10} />
          </div>

          <FormField label="Reason for Leave" type="textarea" required icon="ri-chat-3-line" value={form.reason} onChange={set("reason")} placeholder="Provide brief details regarding reason for leave application..." compact marginBottom={12} />
          ...
          <div style={{ display: "flex", justifyContent: "flex-end", gap: 10, marginTop: 4, paddingTop: 12, borderTop: "1px solid var(--line)" }}>
            ...
            <Button type="submit" disabled={saving} className="btn-glow" style={{ padding: "7px 16px", fontSize: 12.5, fontWeight: 700, display: "inline-flex", alignItems: "center", gap: 6, background: "var(--gradient-primary)" }}>
              {saving ? (<><i className="ri-loader-4-line spin" /> Submitting…</>) : (<><i className="ri-send-plane-line" /> Submit Request</>)}
            </Button>
          </div>
        </form>
      </Modal>
