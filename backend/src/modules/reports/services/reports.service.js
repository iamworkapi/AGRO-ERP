...
async function resolveWarehouseScope(actor, warehouseId) {
  ...
  const effectiveId = actor.profile.role === ROLES.SUPER_ADMIN
    ? warehouseId
    : await getOwnWarehouseId(actor.profile);
...
export async function getDashboardStats(actor, warehouseId) {
  const { filter, effectiveId } = await resolveWarehouseScope(actor, warehouseId);

  if (Object.keys(filter).length === 0 && actor.profile.role === ROLES.SUPER_ADMIN && !warehouseId) {
    const warehouses = await Warehouse.find({}).select("_id name address status commodity");
    ...
    return {
      ...
      recentActivity: [],
      moistureSnapshot: { average: 30, threshold: 14, unit: "%" },
    };
  }
  ...
  const [
    ...
    Warehouse.findById(effectiveId).select("_id name address status commodity"),
    StockEntry.aggregate([
  ...
  const attendanceRate = attendanceAgg[0]?.total
    ? Math.round((attendanceAgg[0].present / attendanceAgg[0].total) * 1000) / 10
    : 0;

  const recentActivity = await StockMovement.find({ warehouse: effectiveId })
    .sort({ createdAt: -1 })
    .limit(5)
    .lean();
  ...
  return {
    ...
    warehouses: ownWarehouse
      ? [{ id: String(ownWarehouse._id), name: ownWarehouse.name, address: ownWarehouse.address || "", commodity: ownWarehouse.commodity || "", status: ownWarehouse.status || "Active" }]
      : [],
    recentActivity: recentActivity.map((m) => ({
      id: String(m._id),
      action: m.action || m.type || "Stock movement",
      time: m.createdAt ? new Date(m.createdAt).toLocaleString("en-IN", { day: "numeric", month: "short", hour: "2-digit", minute: "2-digit" }) : "",
    })),
    moistureSnapshot: { average: 30, threshold: 14, unit: "%" },
  };
}
